			...
			$.fn.extend({
				tyslide:function(opts){
					...
					$(this).each(function(index,item){//此处循环是因为选择器可能选择了页面上多个轮播
						...
						var w2=boxsount*(prams.controlsW+6);//此宽度是controls的宽度
						...
						if(isNumber)
						{
							$(item).find(".controls>li").css({"width":prams.controlsW,"height":prams.controlsH,"border-radius":prams.radius,"padding":"0","display":"block","border":"0","line-height":"30px","font-size":"16px","color":"#fff","text-align":"center"});
						}
						else
						{
